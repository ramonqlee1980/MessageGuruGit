...
-(NSString*)sqliteUnescape
{
    NSMutableString* r = [self mutableCopy];
    NSArray* TEMP = @[@"/", @"//",@"'", @"''",@"[", @"/[",@"]", @"/]",@"%", @"/%",@"&",@"/&",@"_", @"/_",@"(", @"/(",@")", @"/)"];
    for (int i = 0; i<TEMP.count/2; i+=2) {
        [r replaceOccurrencesOfString:[TEMP objectAtIndex:i+1] withString:[TEMP objectAtIndex:i] options:NSCaseInsensitiveSearch range:NSMakeRange(0, r.length)];
    }
    return r;
}
@end
