...
- (NSInteger)countOfRecords:(NSString*)table
{
    int count = 0;
    if (sqlite3_open([[self getDatabasePath] UTF8String], &db) == SQLITE_OK)
    {
        const char* sqlStatement = [[NSString stringWithFormat:@"SELECT COUNT(*) FROM %@",table]UTF8String];
        sqlite3_stmt *statement;
...
- (NSString *)getDatabasePath{
    ...
    NSString* resourceDbFile = [NSString stringWithFormat:@"%@/%@",[[NSBundle mainBundle]resourcePath],databaseName];
    if ([[NSFileManager defaultManager]fileExistsAtPath:resourceDbFile]) {
        return resourceDbFile;
    }
